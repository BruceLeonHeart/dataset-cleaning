...
def create_single_annotations( semg_kp_list=[], num_kp=0, area=0, kps=[], image_id=0, bbox_list=[], category_id=0, ann_id=0 ):
    single_annotations = dict(
                                segmentations   = semg_kp_list, # x,y, x,y
    ...
    for index, single_json in enumerate( json_list ):
        with open(single_json, 'r') as json_f:
            ...
            for shape_d in objs_shapes:
                ...
                for index, shaep in enumerate( objs_sh ):
                    label = shaep['label']
                    if label in classify_names:
                        category_id =  classify_names.index(label) + 1
                        ann_segmentation = []
                        person_seg_points = shaep['points']
                        '''
                        (x1, y1), (x2, y2) = points
                        x1, x2 = sorted([x1, x2])
                        y1, y2 = sorted([y1, y2])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_json_path', type=str, default='/home/fisun/dataset/public/coco_ds/single_kp_ds_all_sample/val2017/json',
                        help='load json file path')       

    parser.add_argument('--save_path', type=str,      default='/home/fisun/dataset/public/coco_ds/single_kp_ds_all_sample/val2017/person_keypoints_dsall_val2017.json',
